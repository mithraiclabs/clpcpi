...
impl Position {
    pub fn get_token_liquidity(&self, whirlpool: &Whirlpool) -> TokenRatio {
        ...
        let position_amount_a =
            get_amount_delta_a(sqrt_price_current, sqrt_price_upper, self.liquidity, false)
                .unwrap();
        let position_amount_b =
            get_amount_delta_b(sqrt_price_lower, sqrt_price_current, self.liquidity, false)
                .unwrap();
